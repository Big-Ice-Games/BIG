...
namespace BIG.Workbook
{
    internal class ExcelWorkbookConverter
    {
        ...
        private WorkbookSheet CreateSheet(string workbookName, ISheet sheet)
        {
            ...
            if (!GetHeader(sheet.GetRow(0), out var header))
            {
                throw new InvalidFormatException(
                    $"WorkbookSheet '{sheet.SheetName}' in workbook '{workbookName}' has no workbookHeader. WorkbookHeader should be the first row with only text values.");
            }
        ...
        private static void Assert(string workbookName, ISheet sheet)
        {
            ...
            {
                throw new InvalidFormatException(
                    $"WorkbookSheet '{sheet.SheetName}' from workbook '{workbookName}' has no rows.");
            }
        }
