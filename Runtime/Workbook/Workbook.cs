...
namespace BIG.Workbook
{
    ...
    public struct WorkbookSheet : IEnumerable<WorkbookRow>
    {
        ...
        public WorkbookSheet(string name, WorkbookHeader workbookHeader, List<WorkbookColumn> columns,
            ...
            foreach (WorkbookCell cell in row)
            {
                if (cell.CellType != stringType)
                {
                    throw new InvalidCastException(
                        $"Each cell in WorkbookRow must be a string cell to convert into WorkbookHeader.");
                }
    ...
    public struct WorkbookCell : IEquatable<WorkbookCell>
    {
        ...
        public override int GetHashCode()
        {
            unchecked
            {
                return ((CellType != null ? CellType.GetHashCode() : 0) * 397) ^
