...
namespace BIG
{
    ...
    public sealed class God
    {
        ...
        private static God Instance => _instance;
        ...
        public God WithLogger(ILogger? logger)
        {
            if (logger != null)
            {
                Instance._builder.Register(_ => logger)
                    .As<ILogger>()
        ...
        /// <summary>
        /// Register modules in priority order — HIGHER priority registers EARLIER (BIG-wide convention).
        /// </summary>
        public God WithAssemblyModules(IList<IAssemblyModule> assemblyModules)
        {
            assemblyModules.OrderByDescending(s => s.Priority).Each(s =>
            {
                Instance._modules++;
        ...
        /// <summary>
        /// Dispose all types registered as IDisposable and reset the container (also official way to reset state in tests).
        /// </summary>
        public static void Dispose()
        {
            ...
            if (container != null)
            {
                if (container.TryResolve<IList<IDisposable>>(out var disposables))
                    disposables.Each(c => c?.Dispose());
        ...
        private void StandaloneRegistration()
        {
            foreach (TypeWithAttribute<RegisterAttribute> t in ReflectionExtension.GetAllTypesByAttribute<RegisterAttribute>())
            {
                ...
                // I notice that sometimes this interface is ignored by AsImplementedInterfaces function.
                if (t.Type.GetInterfaces().Contains(typeof(IDisposable)))
                    build = build.As<IDisposable>();

                if (t.Attribute.Singleton)
                    build = build.SingleInstance();
